...
`GET /api/auth/signals/stream`: `text/event-stream`, кадр `event: {name}\ndata: {json}\n\n`, heartbeat `: ping\n\n` каждые **25 с**, события `connected` (connectionId), `message`, `notification`, `notification_removed` (social group emptied — payload `{ notificationUuid, groupKey? }`; no FCM notification block, companion data-only FCM `type=notification_dismiss` + `tag`), `presence`, `typing`, `read` (conversation marked read by peer — metadata only, no FCM). Хаб in-memory (per-user / per-connection каналы) — без брокера; presence watch scoped to SSE connection. Clients that do not understand `notification_removed` ignore the frame (additive).
