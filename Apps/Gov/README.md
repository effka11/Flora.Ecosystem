...
Браузер ходит только на Next (`:3001`); Next проксирует `/api/auth/*` и `/api/messaging/*` на `flora-api`. Регистрация остаётся в Social (`https://social.flora-s.net` в проде, `http://localhost:3000` локально).
...
Хост: `gov.flora-s.net` (Cloudflare orange, как `social.*`). nginx vhost (прокси на `:3001`) поднимает `Apps/Web/scripts/remote-bootstrap-flora-web.sh` вместе с Social. CORS `https://social.flora-s.net` и `https://gov.flora-s.net` пишет тот же bootstrap в `flora-api-cors.env`. Серый `origin.*` в CORS нет: это origin-pull Cloudflare, не публичный клиентский хост.

Сборка и systemd Next Gov — отдельный деплой (`flora-gov` на `127.0.0.1:3001`). Не переписывает nginx и не трогает Social/`flora-api`:

```powershell
.\Scripts\deploy-flora-gov.ps1
```

Пока процесса нет, origin отвечает 502. Регистрация остаётся в Social; модерация на Gov требует franking-ревьюера (`Scripts/grant-franking-reviewer.ps1`).
