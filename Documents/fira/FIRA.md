...
> Изменения 0.3 → 0.4: **User Controls v1.1 выполнены** (Rust-сторона): настройки ленты, «не интересно» (посты/люди/сообщества/треки), скрытие авторов, seen-post demotion — нормативные разделы в компонентных спеках (FIRA-F §User Controls; FIRA-P/C/M §Feedback Loop). Дорожная карта §17 и инварианты суверенитета §16 обновлены. Паритетная поверхность не расширена: нейтральные дефолты дают бит-в-бит прежние Score, golden-вектора не регенерировались.
>
...
2. **Privacy-first.** Сигналы строятся исключительно на публичном поведении и явных предпочтениях. E2E-переписка никогда не читается и не анализируется. Протокол чата: [`Documents/fscp/FSCP.md`](../fscp/FSCP.md). Платформенная модель и границы приватности: [`Documents/fscp/e2e-security.md`](../fscp/e2e-security.md), раздел «Поиск и рекомендации без plaintext чатов».
...
Подробная модель угроз и криптографические гарантии: [`Documents/fscp/e2e-security.md`](../fscp/e2e-security.md).
...
2. **Настройки пользователя сильнее глобальных дефолтов** (FGP §1.2 п. 2): никакое изменение параметров FIRA — включая будущие governance-решения — не переопределяет пользовательские настройки (явные интересы, скрытия, отключение репостов в рекомендациях, dismissal). Реализация v1.1 — FIRA-F §User Controls: per-user настройки применяются к конфигу через `apply_preferences` **после** глобальных дефолтов и полностью их перекрывают.
...
| **v1.1** «гигиена» | **Выполнено:** фильтр приватных сообществ во всех источниках; блоклист в пулах F/P; точный интерливинг ε (`period = round(1/ε − 1)`); детерминированный trending-префикс (`ORDER BY` до `Take`); строгий author-diversity; golden-вектора сняты после этих правок, Rust-порты скореров закреплены на них (§15). **Выполнено (User Controls, Rust-сторона):** настройки ленты FIRA-F (`freshness`/`exploration`/`showReposts`/`communityPosts`/`seenPosts`/`authorDiversity`, FIRA-F §User Controls); «не интересно» по посту + штраф автора; скрытие авторов; dismissal C/P/M; seen-post demotion/hide. Нейтральные дефолты сохраняют паритет — вектора не регенерировались. **Остаток:** fallback-сортировки C/M (§16) | немедленно, пока модули в статусе «не начат» (§6.0); правки формул требуют регенерации векторов и синхронного обновления Rust-портов |
