...
use flora_image_codec::{
    decode, encode, read_info, DecodedImage, EncodeError, EncodeMode, ImageView, PixelFormat,
};
...
fn synthetic(width: u32, height: u32, format: PixelFormat) -> Vec<u8> {
    ...
    let mut seed = 0xF10_2A_2025u64 ^ (width as u64) << 32 ^ height as u64;
...
fn view(width: u32, height: u32, format: PixelFormat, data: &[u8]) -> ImageView<'_> {
    ImageView { width, height, format, data }
}
...
fn lossless_exact_various_sizes_rgb() {
    ...
    for &(w, h) in
        &[(1, 1), (3, 3), (8, 8), (13, 9), (255, 257), (256, 256), (300, 100), (511, 2)]
    {
...
fn lossless_flat_image_is_tiny() {
    ...
    assert!(fic.len() < 4096, "плоское изображение заняло {} байт", fic.len());
}
...
fn lossy_quality_thresholds() {
    ...
    for (quality, min_psnr) in [(50u8, 28.0), (75, 30.0), (90, 33.0)] {
        let fic = encode(&view(w, h, PixelFormat::Rgb8, &data), EncodeMode::Lossy { quality })
            .unwrap();
        let out = decode(&fic).unwrap();
...
fn lossy_size_monotonic_in_quality() {
    ...
    let sizes: Vec<usize> = [30u8, 70, 95]
        .iter()
        .map(|&q| {
            encode(&view(w, h, PixelFormat::Rgb8, &data), EncodeMode::Lossy { quality: q })
                .unwrap()
                .len()
        })
        .collect();
    assert!(sizes[0] < sizes[1] && sizes[1] < sizes[2], "размеры не монотонны: {sizes:?}");
}
...
fn lossy_alpha_stays_lossless() {
    ...
    let data = synthetic(w, h, PixelFormat::Rgba8);
    let fic =
        encode(&view(w, h, PixelFormat::Rgba8, &data), EncodeMode::Lossy { quality: 60 }).unwrap();
    let out: DecodedImage = decode(&fic).unwrap();
    assert_eq!(out.format, PixelFormat::Rgba8);
    for (i, (src, dec)) in data.chunks_exact(4).zip(out.data.chunks_exact(4)).enumerate() {
...
fn info_matches_encode_parameters() {
    ...
    assert!(!info.lossless && info.chroma420, "q=70 должен включать 4:2:0");
...
fn encode_input_validation() {
    let data = vec![0u8; 12];
    assert!(matches!(
        encode(&view(2, 2, PixelFormat::Rgb8, &data[..11]), EncodeMode::Lossless),
        Err(EncodeError::BufferSizeMismatch { expected: 12, actual: 11 })
    ));
    assert!(matches!(
    ...
    assert!(matches!(
        encode(&view(2, 2, PixelFormat::Rgb8, &data), EncodeMode::Lossy { quality: 0 }),
        Err(EncodeError::InvalidQuality(0))
    ));
    assert!(matches!(
        encode(&view(2, 2, PixelFormat::Rgb8, &data), EncodeMode::Lossy { quality: 101 }),
