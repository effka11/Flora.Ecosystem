...
mod tests {
    ...
    const ORGANIZER: &str = "/api/chat-organizer";
    ...
    async fn blocked_caller_is_denied_writes_but_keeps_reads() {
        ...
        let (status, body) = send(&router, "PUT", ORGANIZER, caller).await;
        assert_eq!(status, StatusCode::FORBIDDEN);
        assert_eq!(body, r#"{"error":"Аккаунт заблокирован."}"#);

        let (status, body) = send(&router, "POST", ORGANIZER, caller).await;
        assert_eq!(status, StatusCode::FORBIDDEN);
        assert_eq!(body, r#"{"error":"Аккаунт заблокирован."}"#);

        let (status, body) = send(&router, "GET", ORGANIZER, caller).await;
    ...
    async fn request_without_current_user_passes_through() {
        ...
        let (status, body) = send(&router, "PUT", ORGANIZER, None).await;
        assert_eq!(status, StatusCode::OK);
        assert_eq!(body, "saved");
    }
    ...
    async fn status_error_fails_closed() {
        ...
        let (status, _) = send(&router, "PUT", ORGANIZER, caller).await;
        assert_eq!(status, StatusCode::INTERNAL_SERVER_ERROR);

        let (status, _) = send(&router, "GET", ORGANIZER, caller).await;
        assert_eq!(status, StatusCode::OK);
    }
}
