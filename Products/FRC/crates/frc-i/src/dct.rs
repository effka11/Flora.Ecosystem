...
// --- прямоугольные блоки W×H (линия v8) -----------------------------------------
//
// Раскладка буферов row-major: `spatial[y*W+x]`, `freq[u*W+v]`, где W —
// ширина (число столбцов), H — высота. Вертикальный базис имеет размер H,
// горизонтальный — W; используются те же литеральные DCT/ADST-базисы,
// что и у квадратных блоков (детерминизм golden-векторов сохраняется).

/// Серпантинный зигзаг для блока W×H (обход диагоналей u+v, чётные —
/// снизу-вверх). На квадрате совпадает с классическим правилом 8×8.
const fn zigzag_rect<const W: usize, const H: usize, const LEN: usize>() -> [usize; LEN] {
    let mut out = [0usize; LEN];
    let mut i = 0usize;
    let mut d = 0usize;
    while d <= W + H - 2 {
        if d.is_multiple_of(2) {
            let mut u = if d < H { d } else { H - 1 };
            loop {
                let v = d - u;
                if v < W {
                    out[i] = u * W + v;
...
pub static ZIGZAG_32X16: [usize; 512] = zigzag_rect::<32, 16, 512>();
pub static ZIGZAG_16X32: [usize; 512] = zigzag_rect::<16, 32, 512>();
pub static ZIGZAG_16X8: [usize; 128] = zigzag_rect::<16, 8, 128>();
pub static ZIGZAG_8X16: [usize; 128] = zigzag_rect::<8, 16, 128>();

/// Матрица квантования W×H из 8×8: коэффициент (u,v) наследует шаг
/// пространственной частоты (u·8/H, v·8/W) — то же правило относительной
/// частоты, что и у квадратных блоков (нормативно для v8).
pub fn quant_matrix_rect<const W: usize, const H: usize, const LEN: usize>(
    q8: &[u16; 64],
) -> [u16; LEN] {
    ...
    for u in 0..H {
        for v in 0..W {
            out[u * W + v] = q8[(u * 8 / H) * 8 + (v * 8 / W)];
        }
    }
    out
}

fn forward_separable_rect<const W: usize, const H: usize, const LEN: usize>(
    spatial: &[f32; LEN],
    freq: &mut [f32; LEN],
    vertical: &[[f32; H]; H],
    horizontal: &[[f32; W]; W],
    ...
    for u in 0..H {
        for x in (0..W).step_by(lanes) {
            ...
            for y in 0..H {
                ...
                for lane in 0..lanes {
                    acc[lane] += weight * spatial[y * W + x + lane];
                }
            }
            for lane in 0..lanes {
                tmp[u * W + x + lane] = acc[lane];
            }
        }
    }
    for u in 0..H {
        for v in (0..W).step_by(lanes) {
            let mut acc = [0f32; 8];
            for x in 0..W {
                let value = tmp[u * W + x];
                for lane in 0..lanes {
            ...
            for lane in 0..lanes {
                freq[u * W + v + lane] = acc[lane];
            }
        }
    }
}

fn inverse_separable_rect<const W: usize, const H: usize, const LEN: usize>(
    freq: &[f32; LEN],
    spatial: &mut [f32; LEN],
    vertical: &[[f32; H]; H],
    horizontal: &[[f32; W]; W],
    ...
    for y in 0..H {
        for x in (0..W).step_by(lanes) {
            ...
            for u in 0..H {
                let weight = vertical[u][y];
                for lane in 0..lanes {
                    acc[lane] += weight * tmp[u * W + x + lane];
                }
            }
            for lane in 0..lanes {
                spatial[y * W + x + lane] = acc[lane];
            }
        }
    }
}

/// Прямое/обратное преобразование W×H с выбором DCT/ADST по осям
/// (`TX_*`, §7.13): вертикальный базис H, горизонтальный W.
macro_rules! rect_tx_fns {
    ($fwd:ident, $inv:ident, $W:literal, $H:literal, $LEN:literal, $vdct:expr, $vadst:expr, $hdct:expr, $hadst:expr) => {
        pub fn $fwd(spatial: &[f32; $LEN], freq: &mut [f32; $LEN], tx: u8) {
            let (vertical, horizontal) = match tx {
                TX_DCT_DCT => ($vdct, $hdct),
                TX_ADST_DCT => ($vadst, $hdct),
                TX_DCT_ADST => ($vdct, $hadst),
                TX_ADST_ADST => ($vadst, $hadst),
                _ => unreachable!("transform ID validated by caller"),
            };
            forward_separable_rect::<$W, $H, $LEN>(spatial, freq, vertical, horizontal);
        }

        pub fn $inv(freq: &[f32; $LEN], spatial: &mut [f32; $LEN], tx: u8) {
            let (vertical, horizontal) = match tx {
                TX_DCT_DCT => ($vdct, $hdct),
                TX_ADST_DCT => ($vadst, $hdct),
                TX_DCT_ADST => ($vdct, $hadst),
            ...
            inverse_separable_rect::<$W, $H, $LEN>(freq, spatial, vertical, horizontal);
        }
    };
}
...
mod tests {
    ...
    #[test]
    fn rect_zigzags_are_permutations_and_square_consistent() {
        fn assert_perm<const LEN: usize>(scan: &[usize; LEN]) {
        ...
        assert_perm(&ZIGZAG_32X16);
        assert_perm(&ZIGZAG_16X32);
        assert_perm(&ZIGZAG_16X8);
        assert_perm(&ZIGZAG_8X16);
        // Правило на квадрате совпадает с классическим зигзагом 8×8.
        let square: [usize; 64] = zigzag_rect::<8, 8, 64>();
        assert_eq!(square, ZIGZAG);
        // Первые позиции: DC, затем сосед по строке / столбцу.
        assert_eq!(&ZIGZAG_32X16[0..3], &[0, 1, 32]);
        assert_eq!(&ZIGZAG_16X32[0..3], &[0, 1, 16]);
    }

    #[test]
    fn rect_transforms_roundtrip_all_tx() {
        fn assert_rt<const LEN: usize>(
            forward: fn(&[f32; LEN], &mut [f32; LEN], u8),
            inverse: fn(&[f32; LEN], &mut [f32; LEN], u8),
            tolerance: f32,
        ) {
            ...
            for tx in 0..N_TX_V7 {
                ...
                let max_error = spatial
                    ...
                    .map(|(a, b)| (a - b).abs())
                    .fold(0.0f32, f32::max);
                assert!(max_error < tolerance, "tx={tx} LEN={LEN}: {max_error}");
            }
        }
        assert_rt(forward_tx_32x16, inverse_tx_32x16, 0.05);
        assert_rt(forward_tx_16x32, inverse_tx_16x32, 0.05);
        assert_rt(forward_tx_16x8, inverse_tx_16x8, 0.03);
        assert_rt(forward_tx_8x16, inverse_tx_8x16, 0.03);
    }

    #[test]
    fn rect_transform_preserves_energy() {
        // Ортонормальность: сумма квадратов сохраняется (равенство Парсеваля),
        // значит шаг квантования даёт ту же пиксельную ошибку, что у квадратов.
        let mut spatial = [0f32; 512];
        for (i, s) in spatial.iter_mut().enumerate() {
            *s = ((i * 53 + 7) % 256) as f32 - 128.0;
        }
        let mut freq = [0f32; 512];
        forward_tx_32x16(&spatial, &mut freq, TX_DCT_DCT);
        let es: f32 = spatial.iter().map(|v| v * v).sum();
        let ef: f32 = freq.iter().map(|v| v * v).sum();
        assert!((es - ef).abs() / es < 1e-4, "{es} vs {ef}");
    }

    #[test]
    fn quant_matrix_rect_inherits_relative_steps() {
        let q8 = quant_matrix(&BASE_LUMA, 75);
        let q = quant_matrix_rect::<32, 16, 512>(&q8);
        for u in 0..16 {
            for v in 0..32 {
                assert_eq!(q[u * 32 + v], q8[(u / 2) * 8 + v / 4]);
            }
        }
        let q = quant_matrix_rect::<8, 16, 128>(&q8);
        for u in 0..16 {
            for v in 0..8 {
                assert_eq!(q[u * 8 + v], q8[(u / 2) * 8 + v]);
            }
        }
        // Правило на квадрате совпадает с quant_matrix16.
        let q16 = quant_matrix_rect::<16, 16, 256>(&q8);
        assert_eq!(q16, quant_matrix16(&q8));
    }
