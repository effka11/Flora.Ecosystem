...
/// FourCC потока FRV1 (контейнер IVF).
pub const FOURCC: [u8; 4] = *b"FRV1";
...
pub enum Error {
    ...
    /// Битстрим повреждён или не является потоком FRC-V / FRV1.
...
impl core::fmt::Display for Error {
    fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        match self {
            ...
            Error::InvalidBitstream(m) => write!(f, "invalid FRC-V bitstream: {m}"),
