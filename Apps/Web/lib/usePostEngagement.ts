...
import {
  ...
  apiUnlikePost,
  apiUnrepostPost,
  primeDevPostEngagement,
  type PostEngagementSnapshot,
} from "@/lib/socialApi";
...
export function usePostEngagement(options: UsePostEngagementOptions = {}) {
  ...
  const toggleLike = useCallback(
    async (post: PostEngagementSource) => {
      ...
      try {
        primeDevPostEngagement(post.postUuid, before);
  ...
  const toggleRepost = useCallback(
    async (post: PostEngagementSource) => {
      ...
      try {
        primeDevPostEngagement(post.postUuid, before);
        const result = nextReposted ? await apiRepostPost(post.postUuid) : await apiUnrepostPost(post.postUuid);
        applySnapshot(post.postUuid, { ...before, reposted: result.reposted, repostsCount: result.repostsCount });
      } catch {
        applySnapshot(post.postUuid, before);
      } finally {
        setPending((p) => {
