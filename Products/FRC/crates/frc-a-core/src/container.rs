...
impl Header {
    ...
    pub fn parse(bytes: &[u8]) -> Result<Self, Error> {
        ...
        if bytes[0..4] != MAGIC {
            return Err(Error::InvalidPacket("bad FRAS magic"));
        }
        if bytes[4] != VERSION {
            return Err(Error::InvalidPacket("unsupported FRAS version"));
        }
