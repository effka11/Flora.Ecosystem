...
mod tests {
    ...
    /// Формы маршрутов Users: GET `/me`, PATCH privacy, POST follow.
    fn app(status: Arc<dyn AccountSanctionStatus>) -> Router {
        write_gate(
            Router::new()
                ...
                .route(
                    "/api/auth/me/privacy",
                    get(|| async { "privacy" }).patch(|| async { "updated" }),
                )
                .route(
    ...
    async fn blocked_caller_is_denied_writes_but_keeps_reads() {
        ...
        let (status, body) = send(&router, "PATCH", "/api/auth/me/privacy", caller).await;
        assert_eq!(status, StatusCode::FORBIDDEN);
        assert_eq!(body, r#"{"error":"Аккаунт заблокирован."}"#);

        let (status, body) = send(&router, "POST", "/api/auth/profile/bob/follow", caller).await;
        assert_eq!(status, StatusCode::FORBIDDEN);
        assert_eq!(body, r#"{"error":"Аккаунт заблокирован."}"#);

        let (status, body) = send(&router, "GET", "/api/auth/me", caller).await;
        assert_eq!(status, StatusCode::OK);
        assert_eq!(body, "me");

        let (status, _) = send(&router, "GET", "/api/auth/me/privacy", caller).await;
        assert_eq!(status, StatusCode::OK);
    }
    ...
    async fn unblocked_caller_writes_normally() {
        ...
        let (status, _) = send(&router, "POST", "/api/auth/profile/bob/follow", caller).await;
        assert_eq!(status, StatusCode::OK);
    }
    ...
    async fn request_without_current_user_passes_through() {
        ...
        let (status, body) = send(&router, "PATCH", "/api/auth/me/privacy", None).await;
        assert_eq!(status, StatusCode::OK);
        assert_eq!(body, "updated");
    }
    ...
    async fn status_error_fails_closed() {
        ...
        let (status, _) = send(&router, "PATCH", "/api/auth/me/privacy", caller).await;
        assert_eq!(status, StatusCode::INTERNAL_SERVER_ERROR);

        let (status, _) = send(&router, "GET", "/api/auth/me", caller).await;
        assert_eq!(status, StatusCode::OK);
    }
}
