//! Golden-вектора заморозки битстрима FRV1 v1 (паттерн FRC-I).
//!
//! Пины:
//! 1. **Энкодер**: на эталонном входе выдаёт байт-в-байт зафиксированный поток
//!    (файлы `tests/data/*.frv`, нативный контейнер — заморожен вместе с битстримом).
...
use frc_v::container::{FrcVHeader, FrcVReader, FrcVWriter};
...
fn update_mode() -> bool {
    std::env::var_os("FRC_V_UPDATE_GOLDEN").is_some_and(|v| v == "1")
}
...
fn encode_vector(v: &Vector) -> Vec<u8> {
    ...
    let mut w = FrcVWriter::new(Cursor::new(Vec::new()), header).expect("in-memory write");
...
fn decode_checksums(bytes: &[u8]) -> Vec<u64> {
    let mut r = FrcVReader::new(bytes).expect("golden container");
...
fn golden_vectors() {
    ...
    if update_mode() {
        ...
        for v in vectors() {
            ...
            std::fs::write(dir.join(format!("{}.frv", v.name)), &bytes).expect("write golden");
    ...
    let sums_text = std::fs::read_to_string(&sums_path)
        .expect("tests/data/golden.sums missing — run with FRC_V_UPDATE_GOLDEN=1 once");
    for v in vectors() {
        let path = dir.join(format!("{}.frv", v.name));
        let stored = std::fs::read(&path).unwrap_or_else(|_| {
            panic!(
                "{} missing — run with FRC_V_UPDATE_GOLDEN=1 once",
                path.display()
            )
        });
