...
/**
 * Зарезервированные ссылки сообществ. Держать в синхроне с
 * `Modules/Flora.Content/Flora.Content.Domain/CommunityReservedSlugs.cs`.
 */
...
const RESERVED_COMMUNITY_SLUG_PREFIXES = [
  "admin",
  "administrator",
  "support",
  "helpdesk",
...
export const RESERVED_COMMUNITY_SLUG_MESSAGE =
  "Эта ссылка зарезервирована системой и недоступна для регистрации.";

function collapseSlugSeparators(slug: string): string {
  return slug.replace(/[-_]/g, "");
}

export function isReservedCommunitySlug(raw: string): boolean {
  const normalized = normalizeCommunitySlug(raw);
  if (!normalized) return true;
  if (RESERVED_COMMUNITY_SLUGS.has(normalized)) return true;
  const collapsed = collapseSlugSeparators(normalized);
  if (collapsed !== normalized && RESERVED_COMMUNITY_SLUGS.has(collapsed)) return true;
  return RESERVED_COMMUNITY_SLUG_PREFIXES.some((prefix) => normalized.startsWith(prefix));
}
