...
import { GOV_NAV_ITEMS } from "./govNavigation";
import styles from "./govShell.module.css";
import { useViewportFrameCssVars } from "./viewportFrame";

type TabIndicatorStyle = CSSProperties & {
  "--gov-tab-indicator-left": string;
  "--gov-tab-indicator-width": string;
};

export type GovShellProps = {
  children: ReactNode;
...
function isGovTabActive(pathname: string, href: string) {
...
function GovNavTabs() {
  ...
  const motionPrimedRef = useRef(false);
  const [motionEnabled, setMotionEnabled] = useState(false);
  const [indicator, setIndicator] = useState({ left: 0, width: 0 });

  const activeIndex = GOV_NAV_ITEMS.findIndex((item) => isGovTabActive(pathname, item.href));

  const syncIndicator = useCallback(() => {
    const target = activeIndex >= 0 ? tabRefs.current[activeIndex] : null;
    if (!target) {
    ...
    if (width <= 0) return;
    setIndicator({ left: target.offsetLeft, width });
  }, [activeIndex]);
  ...
  useEffect(() => {
    ...
    const observer = typeof ResizeObserver !== "undefined" ? new ResizeObserver(syncIndicator) : null;
    if (observer) {
  ...
  useEffect(() => {
    if (motionPrimedRef.current || indicator.width <= 0) return;
    motionPrimedRef.current = true;
    const frame = requestAnimationFrame(() => setMotionEnabled(true));
    return () => cancelAnimationFrame(frame);
  }, [indicator.width]);

  const indicatorStyle: TabIndicatorStyle = {
    "--gov-tab-indicator-left": `${indicator.left}px`,
    "--gov-tab-indicator-width": `${indicator.width}px`,
  };

  return (
    <nav className={styles.nav} aria-label="Разделы">
      <div className={styles.navTabsWrap}>
        <div ref={rowRef} className={styles.navTabs}>
          {GOV_NAV_ITEMS.map((item, index) => {
          ...
          <div
            className={`${styles.navIndicator} ${motionEnabled ? "" : styles.navIndicatorStatic}`}
...
function GovShellInner({ children, statusSlot }: GovShellProps) {
    ...
    <div className={styles.page}>
      ...
      <header className={styles.header}>
        <div className={styles.headerInner}>
          <Link href="/overview" className={styles.logo} aria-label="Flora Gov">
            <FloraLogoMark />
            <span className={styles.logoText}>FLORA</span>
            <span className={styles.logoSuffix}>GOV</span>
          </Link>
          {statusSlot ? <div className={styles.status}>{statusSlot}</div> : null}
        </div>
        <GovNavTabs />
      </header>
...
export function GovShell(props: GovShellProps) {
  return <GovShellInner {...props} />;
}
