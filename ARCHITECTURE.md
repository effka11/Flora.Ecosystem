...
**Apps/** остаются в корне (shells: Web, Www, Mobile, Gov). Запрещено `Products/*` → `Apps/*`. Три слоя клиента: shell → optional App `client/` → functional TS/wasm SoT в продукте; `@flora/client-core` — транспорт + реэкспорт.
