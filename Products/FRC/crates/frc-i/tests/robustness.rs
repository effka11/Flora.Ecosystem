...
fn sample_streams() -> Vec<Vec<u8>> {
    ...
    vec![
        encode(&g, EncodeMode::Lossy { quality: 60 }).unwrap(), // v10 (текущий, asymmetric AQ)
        encode(&g, EncodeMode::Lossy { quality: 30 }).unwrap(), // v10 + деблокинг
        encode(&g, EncodeMode::Lossless).unwrap(),
        encode(&f, EncodeMode::Lossless).unwrap(), // палитра (2 цвета)
        encode_with_icc(&g, EncodeMode::Lossy { quality: 60 }, &[1, 2, 3, 4]).unwrap(), // v10+meta
        encode_with_version(&g, EncodeMode::Lossy { quality: 60 }, 9).unwrap(), // v9 (delta-Q)
        encode_with_version(&g, EncodeMode::Lossy { quality: 60 }, 8).unwrap(), // v8 (YCoCg)
...
fn garbage_with_valid_magic_never_panics() {
    ...
    for version in [1u8, 2, 3, 4, 5, 6, 7, 8, 9, 10] {
        for _ in 0..500 {
