# Бенчмарк FRC-V против x264 (tune psnr) на y4m-клипе с одинаковым GOP.
...
$frcV = Join-Path $PSScriptRoot "..\target\release\frc-v.exe"
...
function PsnrOf($dist) {
    ...
    $out = & $frcV psnr --ref $src --dist $dist
...
foreach ($qp in 16, 24, 32, 40, 48) {
    ...
    $enc = Join-Path $tmp "$name-frc-v-q$qp.frv"

    $dec = Join-Path $tmp "$name-frc-v-q$qp.y4m"

    & $frcV encode -i $src -o $enc --qp $qp --keyint $Keyint | Out-Null

    & $frcV decode -i $enc -o $dec | Out-Null
    ...
    Write-Output "frc-v,qp$qp,$bytes,$bpp,$psnr"
...
# (FRC-V v1 — только P-кадры с одной опорой; сравнение честно по структуре GOP).
