...
Планируемые наборы governance/personhood (появятся вместе с реализацией `flora-governance-crypto`; состав и форматы нормированы в [fgp/FGP-CRYPTO.md](../fgp/FGP-CRYPTO.md) §12): `governance/` — деривации и nullifier'ы, DS-метки, VRF/самовыборка, токены кредитов, транскрипт тэлли, журнал (inclusion/consistency), фикс-пойнт таблицы; `personhood/` — по [fpp/FPP.md](../fpp/FPP.md) §10.4. Правила ниже (base64url, `protocolVersion`, негативы, обязательные consumer-тесты) распространяются и на них.
...
`fscp-rke-wrap-key-v1.json`, `fscp-message-transcript-v1.json`, `franking-v1.json` и `fscp-hybrid-kem-v2draft-v1.json` — из каталога `docs/test-vectors/` (нужны `cryptography`, `PyNaCl`; для гибридного KEM — ещё `kyber-py`; franking читает транскрипт — порядок важен):
