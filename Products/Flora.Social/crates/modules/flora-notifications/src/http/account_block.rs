...
mod tests {
    ...
    /// Protected inbox forms: GET list + DELETE (write) — без полного NotificationsState.
    ...
    async fn blocked_caller_is_denied_writes_but_keeps_reads() {
        ...
        let (status, body) = send(&router, "DELETE", "/api/auth/notifications", caller).await;
        assert_eq!(status, StatusCode::FORBIDDEN);
        assert_eq!(body, r#"{"error":"Аккаунт заблокирован."}"#);

        let (status, body) = send(&router, "GET", "/api/auth/notifications", caller).await;
    ...
    async fn request_without_current_user_passes_through() {
        ...
        let (status, body) = send(&router, "DELETE", "/api/auth/notifications", None).await;
        assert_eq!(status, StatusCode::OK);
        assert_eq!(body, "deleted");
    }
    ...
    async fn status_error_fails_closed() {
        ...
        let (status, _) = send(&router, "DELETE", "/api/auth/notifications", caller).await;
        assert_eq!(status, StatusCode::INTERNAL_SERVER_ERROR);

        let (status, _) = send(&router, "GET", "/api/auth/notifications", caller).await;
        assert_eq!(status, StatusCode::OK);
    }
}
