...
fn csharp_created_hashes_verify_in_rust() {
    let vector = load_json(&golden_vectors_dir().join("argon2id-v1.json"))
        .expect("нет argon2id-v1.json — сгенерируйте из C#: ./Scripts/generate-golden-vectors.ps1");
    ...
    for case in vector["cases"].as_array().unwrap() {
        ...
        assert!(
            verify_like_csharp(password, stored_hash),
            "пароль {password:?} отвергнут"
        );
        println!("argon2id verify ({password:?}): {:?}", started.elapsed());

        assert!(!verify_like_csharp(
