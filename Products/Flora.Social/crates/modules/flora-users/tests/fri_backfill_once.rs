...
//! `cargo test -p flora-users --test fri_backfill_once -- --ignored --nocapture`

use sqlx::postgres::PgPoolOptions;

#[tokio::test]
#[ignore = "mutates local flora_social DB"]
async fn backfill_user_avatars_to_fri() {
    ...
    sqlx::query("SET search_path TO flora_core")
        ...
        .await
        .expect("search_path");
    flora_users::infrastructure::image_backfill::run(pool).await;
}
