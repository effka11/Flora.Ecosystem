...
pub fn messaging_ports(
    ...
    let profiles: Arc<dyn flora_users_contracts::FeedAuthorProfiles> =
        Arc::new(infrastructure::social_graph::SqlSocialGraph::new(pool.clone()));
    let online: Arc<dyn flora_users_contracts::OnlineStatusAccess> =
        Arc::new(infrastructure::online_access::SqlOnlineStatusAccess::new(pool.clone()));
    let messages: Arc<dyn flora_users_contracts::MessagesAccess> =
        Arc::new(infrastructure::messages_access::SqlMessagesAccess::new(pool));
...
pub fn profile_queries(
    pool: PgPool,
) -> Arc<dyn flora_users_contracts::UserProfileQueries> {
...
pub fn compose(
    ...
    let messages_access: Arc<dyn flora_users_contracts::MessagesAccess> =
        Arc::new(infrastructure::messages_access::SqlMessagesAccess::new(pool.clone()));
    let profile_access: Arc<dyn flora_users_contracts::ProfileAccess> =
        Arc::new(infrastructure::profile_access::SqlProfileAccess::new(pool.clone()));
