/**
 * UI mute entry (forever / until). Server overlay хранит только boolean mutedByPeer;
 * until — локальный countdown, без auto-unmute на сервере.
 */
...
export function formatConversationMuteTooltip(
  entry: ConversationMuteEntry,
  nowMs = Date.now(),
): string {
