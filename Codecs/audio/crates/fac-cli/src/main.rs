//! Инструмент разработки FAC: генерация тестовых сигналов, кодирование,
//! декодирование и roundtrip-метрики. Спецификация: docs/codecs/FAC.md.
...
use fac_core::{Config, Decoder, Encoder, FRAME_N, container};
...
#[command(
    name = "fac",
    version,
    about = "FAC (Flora Audio Codec) — инструмент разработки"
)]
struct Cli {
    #[command(subcommand)]
    cmd: Cmd,
...
fn decode(input: &std::path::Path, output: &std::path::Path) -> CliResult<()> {
    ...
    while pos + 2 <= bytes.len() {
        ...
        if pos + len > bytes.len() {
            return Err(fac_core::Error::Truncated.into());
        }
