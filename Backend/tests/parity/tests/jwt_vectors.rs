...
use flora_auth::infrastructure::jwt::{
    issue_access_token, validate_access_token, AccessTokenClaims, JwtOptions,
};
use flora_parity::{golden_vectors_dir, load_json};

fn vector() -> serde_json::Value {
    load_json(&golden_vectors_dir().join("jwt-hs256-v1.json")).expect(
        "нет jwt-hs256-v1.json — сгенерируйте из C#: ./Scripts/generate-golden-vectors.ps1",
    )
}
...
fn csharp_issued_token_validates_in_rust() {
    ...
    assert_eq!(claims.expires_at, vector["payload"]["exp"].as_i64().unwrap());
}
...
fn rust_issued_token_reproduces_csharp_wire_payload_exactly() {
    ...
    let decode = |part: &str| -> serde_json::Value {
        ...
        let bytes = base64::engine::general_purpose::URL_SAFE_NO_PAD.decode(part).unwrap();
        serde_json::from_slice(&bytes).unwrap()
    };
    let rust_parts: Vec<&str> = token.split('.').collect();
    let csharp_parts: Vec<&str> = vector["token"].as_str().unwrap().split('.').collect();

    assert_eq!(decode(rust_parts[0]), decode(csharp_parts[0]), "JOSE header");
    assert_eq!(decode(rust_parts[1]), decode(csharp_parts[1]), "wire-набор клеймов");
