//! WASM-декодер FRV1 для Apps/Web (`wasm32-unknown-unknown`, без wasm-bindgen).
//!
...
//! frc_v_alloc(len) -> ptr                      буфер под входной пакет
...
//! (только для отображения). JS-глу: см. `www/frc-v-player.mjs` рядом с crate.
//!
//! `unsafe` ограничен границей FFI (см. Cargo.toml); инварианты каждого блока
//! описаны на месте. Ядро декодера — безопасный `frc_v`.

use frc_v::{Decoder, Frame};
