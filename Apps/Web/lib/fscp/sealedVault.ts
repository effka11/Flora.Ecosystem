/**
 * At-rest vault for FSCP device profiles (SEC-1).
 * Non-extractable AES-GCM wrap key + sealed profile ciphertext in IndexedDB.
 */

import type { FscpProfileRecord } from "@flora/client-core/fscp";

export const FSCP_VAULT_DB = "flora-fscp-vault";
...
/** IndexedDB / structured-clone may return ArrayBuffer or a TypedArray view. */
...
/** Normalize a value read from IDB into a sealed blob (or null if unusable). */
...
function openDb(): Promise<IDBDatabase> {
  ...
  return new Promise((resolve, reject) => {
    const request = indexedDB.open(FSCP_VAULT_DB, FSCP_VAULT_DB_VERSION);
...
/**
 * Put-if-absent wrap key: concurrent tabs may both generate; re-read wins with the stored key.
 */
...
export function getDefaultFscpSealedVault(): FscpSealedVault {
  if (!defaultVault) defaultVault = createIndexedDbFscpSealedVault();
  return defaultVault;
}

/** Test hook: replace or reset the process-wide default vault. */
export function setDefaultFscpSealedVaultForTests(vault: FscpSealedVault | null): void {
  defaultVault = vault;
}
