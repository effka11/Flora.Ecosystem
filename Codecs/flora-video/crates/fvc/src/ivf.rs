//! Контейнер IVF (как у VP8/VP9/AV1) с FourCC `FVC1` — формат разработки.
...
impl<W: Write> IvfWriter<W> {
    pub fn new(mut inner: W, header: IvfHeader) -> io::Result<Self> {
        ...
        inner.write_all(&h)?;
        Ok(IvfWriter { inner, frames_written: 0 })
    }
...
impl<R: Read> IvfReader<R> {
    pub fn new(mut inner: R) -> io::Result<Self> {
        ...
        if header_size != 32 {
            return Err(io::Error::new(io::ErrorKind::InvalidData, "unsupported IVF header size"));
        }
    ...
    pub fn read_frame(&mut self) -> io::Result<Option<(u64, Vec<u8>)>> {
        ...
        if size > 256 * 1024 * 1024 {
            return Err(io::Error::new(io::ErrorKind::InvalidData, "ivf frame size implausible"));
        }
