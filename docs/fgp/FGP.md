...
Компаньоны: [`FGP-THREATS.md`](./FGP-THREATS.md) (v0.3) — модель противников, операционные программы защиты (V-01…V-13), красная команда, симуляции, лестница деградации, кризисные процедуры, исследовательские треки; [`FPP.md`](../fpp/FPP.md) (v0.1) — нормативная спецификация personhood (уровни V0–V3, церемонии, поручительства, якоря, `C_identity`). Все три документа нормативны; при конфликте приоритет у `FGP.md`. История ревью — Приложение B (B.1: 0.1 → 0.2, 25 находок; B.2: 0.2 → 0.3, ещё 25; B.3: 0.3 → 0.4 — согласование с целевым Rust-стеком и юридическим слоем).
...
- **Test vectors:** `docs/test-vectors/governance/` (по образцу FSCP): фикстуры тэлли и ZK-пруфов, nullifier-деривации, VRF-выборок, bridging-скоринга, корреляционного дисконта. Вектора фиксируют и числовые форматы (§8.1) — они и есть спецификация арифметики; паритет server ↔ WASM-клиент проверяется на одном наборе.
