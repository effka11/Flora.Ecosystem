...
Коммерческая лицензия  –  на **тот же код Flora Ecosystem**; её предлагает правообладатель под торговым именем **Luna** (закрытая встройка, OEM, SLA). Это не вторая экосистема, не отмена AGPL для пользовательской Flora и не «закрытие» соцсети для сообщества. SPDX `LicenseRef-Flora-Ecosystem-Commercial` именует произведение, не продавца.

Подробности: [`LEGAL.md`](LEGAL.md), [`LICENSE`](LICENSE), [`LICENSE-COMMERCIAL.md`](LICENSE-COMMERCIAL.md), [`NOTICE.md`](NOTICE.md). Некодовые материалы  –  **CC BY-SA 4.0** или коммерческая лицензия на контент, тоже через Luna ([`LICENSE-CONTENT.md`](LICENSE-CONTENT.md)).
...
Реклама, платное продвижение и платная кастомизация «для статуса» исключены: они ломают доверие быстрее, чем любой баг. Брендовый контент помечается, идёт по тем же правилам, что и обычные посты, и отключается одной настройкой. **Luna**  –  торговое имя, под которым продаётся исключение из AGPL для чужих закрытых интеграций кода; не продажа внимания пользователей Flora и не второй продукт.

Дальше по той же логике проектируется экономика **FEP / LIV**: деньги как средство обмена, а не рычаг власти (спека и ядра есть; в прод пользовательской сети пока не включаем).
